 ...
 *
 * CSS CONTRACT — every class name generated by this file.
 * storyboard-panel-hardware-theme.css MUST only target selectors from this list
 * or from the static HTML in index.html.  Run scripts/check-storyboard-css.sh to verify.
 *
 * Static HTML (index.html): storyboard-panel, storyboard-header, storyboard-title,
 *   storyboard-header-meta, storyboard-body, storyboard-footer,
 *   storyboard-btn, storyboard-btn-primary, storyboard-btn-secondary
 *   IDs: storyboard-overlay, storyboard-panel, storyboard-title, storyboard-meta,
 *        storyboard-body, storyboard-close, storyboard-export-json, storyboard-generate-all,
 *        storyboard-compose-narrative
 *
 * Dynamic (this file):
 *   storyboard-anchors, storyboard-anchors-title, storyboard-anchors-grid
 *   storyboard-anchor-chip, anchor-key, anchor-value
 *   storyboard-characters, storyboard-characters-title, storyboard-characters-grid
 *   storyboard-character-chip, char-id, char-name, char-anchors
 *   storyboard-act-group, storyboard-act-header, act-name, act-beat-count
 *   storyboard-beat-grid
 *   storyboard-beat-card  [data-beat-id]
 *   beat-card-header, beat-number, beat-shot
 *   beat-card-media  [id=beat-media-{n}], beat-thumb, beat-placeholder, beat-placeholder-icon, beat-video-badge
 *   beat-card-purpose, beat-card-prompt
 *   beat-card-narrative, narrative-label, narrative-text
 *   beat-card-chars, beat-char-tag
 *   beat-card-actions, beat-action-btn, beat-regen-btn, beat-img-btn, beat-vid-btn, beat-copy-btn
 *   beat-loading  (toggled on .storyboard-beat-card during regen/image-gen)
 */
...
export class StoryboardPanel {
  ...
  constructor(app) {
    ...
    this.beatNarrative = {}; // beatId → cohesive video-prompt string from narrative service
    this._bound = false;
  ...
  _createBeatCard(entry) {
    ...
    card.innerHTML = `
      ...
      ${this.beatNarrative[entry.beat] ? `
        <div class="beat-card-narrative" title="${this._escapeAttr(this.beatNarrative[entry.beat])}">
          <span class="narrative-label">🎭 Narrative</span>
          <span class="narrative-text">${this._truncate(this.beatNarrative[entry.beat], 140)}</span>
        </div>
      ` : ''}
      ...
      <div class="beat-card-actions">
        <button type="button" class="beat-action-btn beat-regen-btn" data-beat="${entry.beat}" title="Regenerate this beat" aria-label="Regenerate beat ${entry.beat}">🔄</button>
        <button type="button" class="beat-action-btn beat-img-btn" data-beat="${entry.beat}" title="Generate image" aria-label="Generate image for beat ${entry.beat}">🖼️</button>
        <button type="button" class="beat-action-btn beat-vid-btn" data-beat="${entry.beat}" title="Generate video" aria-label="Generate video for beat ${entry.beat}">🎥</button>
        <button type="button" class="beat-action-btn beat-copy-btn" data-beat="${entry.beat}" title="Copy prompt" aria-label="Copy prompt for beat ${entry.beat}">📋</button>
      </div>
    `;
  ...
  async _regenerateBeat(beatId) {
    ...
    if (card) { card.classList.add('beat-loading'); card.setAttribute('aria-busy', 'true'); }

    try {
      // Pass adjacent keyframe images (if generated) so the LLM has visual
      // continuity context, not just the textual prompts of neighbouring beats.
      const stripDataUri = (s) => s ? s.replace(/^data:image\/[^;]+;base64,/, '') : null;
      const prevImage = stripDataUri(this.beatMedia[beatId - 1]?.imageData);
      const nextImage = stripDataUri(this.beatMedia[beatId + 1]?.imageData);
      ...
      const res = await fetch('/api/generate/template', {
        ...
        body: JSON.stringify({
          ...
          prev_image_b64: prevImage,
          next_image_b64: nextImage,
        })
      });
      ...
      if (card) { card.classList.remove('beat-loading'); card.removeAttribute('aria-busy'); }
    }
  }
  ...
  /**
   * Compose a cohesive narrative across all beats by calling the
   * /api/generate/narrative endpoint with each beat's expanded prompt
   * as the visual description and the story logline as the North Star.
   * Returned per-beat video prompts are stored in beatNarrative and
   * rendered on each beat card.
   */
  async _composeNarrative() {
    if (!this.sequence || this.sequence.length === 0) {
      this._toast('No beats to compose narrative from', 'warning');
      return;
    }

    const story = this.template.story || {};
    const logline = story.logline || story.title || '';
    const userPrompt = (typeof window !== 'undefined' && window.prompt)
      ? (window.prompt('North Star — what is this story about? (one sentence)', logline) || logline)
      : logline;
    if (!userPrompt) {
      this._toast('Narrative composition cancelled', 'info');
      return;
    }

    const btn = document.getElementById('storyboard-compose-narrative');
    const originalLabel = btn ? btn.textContent : '';
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Composing…'; }

    try {
      const descriptions = this.sequence.map(e => {
        const parts = [];
        if (e.shot) parts.push(`[${e.shot}]`);
      ...
      if (prompts.length === 0) {
        throw new Error('Narrative service returned no prompts');
      }

      this.sequence.forEach((entry, i) => {
        if (prompts[i]) this.beatNarrative[entry.beat] = String(prompts[i]);
      });

      this._render();
      this._toast(`Narrative composed across ${prompts.length} beats`, 'success');
    } catch (err) {
      console.error('Narrative composition failed:', err);
      this._toast(`Narrative composition failed: ${err.message}`, 'error');
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = originalLabel || '🎭 Compose Narrative'; }
    }
  }
      ...
      acts: (story.acts || []).map(act => {
        ...
        return {
          ...
          beats: actBeats.map(b => ({
            ...
            purpose: b.purpose || '',
            prompt: b.text,
            narrative: this.beatNarrative[b.beat] || null,
  ...
  _bindGlobalEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (e.target.id === 'storyboard-compose-narrative') this._composeNarrative();
    });
