...
class StudioIntegration {
    ...
    /**
     * Timestamp for download filenames: 20260720-143052. Seconds included on
     * purpose — minute precision collides when you download two results in
     * quick succession and the browser silently appends "(1)".
     */
    downloadStamp() {
        const d = new Date();
    ...
    /**
     * Download generated bytes straight from the browser. Deliberately NOT
     * gated like saveArtifact() — the payload is already here, nothing is
     * uploaded, so this works signed-out and on local installs.
     *
     * Goes via Blob rather than an `href="data:…"` anchor: a data URL holds
     * the payload a second time as a string and browsers cap how large one
     * can be, which bites on multi-MB results.
     */
    downloadBase64(base64Data, mime, filename) {
        try {
            ...
            a.click();
            a.remove();
            // Revoking immediately can race the click in some browsers.
            setTimeout(() => URL.revokeObjectURL(url), 1000);
        } catch (e) {
            console.error(e);
            this.showToast('Download failed — try right-clicking the image and saving it instead.', 'error', 4000);
        }
    }

    /** Download one of the current image results by index. */
    downloadImage(index) {
        const b64 = this.currentBatchResults?.[index];
        if (!b64) {
            this.showToast('Nothing to download yet — generate first.', 'warning', 3000);
            return;
        }
        this.downloadBase64(b64, 'image/png', `synthograsizer-${this.downloadStamp()}.png`);
    }
    ...
    async runSingle(type, params) {
        try {
            ...
            if (type === 'image') {
                ...
                // Ungated on purpose — see downloadBase64(). Sits beside Save so the
                // two "keep this file" actions read as a pair.
                html += `<button class="synth-download-image" title="Saves the PNG to your device — nothing is uploaded. Note: the file's metadata carries the prompt that made it, so sharing the image shares the prompt." onclick="window.studioIntegrationInstance.downloadImage(0)" style="background:rgba(120,120,130,0.12); border:1px solid rgba(120,120,130,0.4); color:#5a5a66; padding:6px 14px; border-radius:6px; cursor:pointer; font-size:13px;">⬇ Download</button>`;
