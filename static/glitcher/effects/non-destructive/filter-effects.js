...
import { DitheringFilter } from './new-filters/dithering.js';
...
export class FilterEffects {
  ...
  static apply(imageData, effectType, intensity = 50, options = {}) {
    ...
    switch (baseFilter) {
      ...
      // NEW: Dithering filter
      case 'dithering':
        // Extract dithering-specific parameters from nested structure
        const ditheringOptions = updatedOptions.dithering || {};
        const algorithm = ditheringOptions.algorithm || 'floyd_steinberg';
        const ditheringParams = {
          colorMode: ditheringOptions.colorMode || 'monochrome',
          cgaPalette: parseInt(ditheringOptions.cgaPalette || '0'),
          colorLevels: parseInt(ditheringOptions.colorLevels || '2'),
          levelsPerChannel: parseInt(ditheringOptions.levelsPerChannel || '4'),
          serpentine: ditheringOptions.serpentine === true,
          brightness: parseInt(ditheringOptions.brightness || '0'),
          contrast: parseInt(ditheringOptions.contrast || '0')
        };
        console.log('🎨 Dithering filter params:', { algorithm, ditheringParams });
        return DitheringFilter.apply(imageData, algorithm, intensity, ditheringParams);
