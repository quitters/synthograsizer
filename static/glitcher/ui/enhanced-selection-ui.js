...
export class EnhancedSelectionUI {
  ...
  init() {
    ...
    if (!document.body.matches('.light-workspace, .classic-workspace')) {
  ...
  createModeIndicator() {
    // Mode indicator removed to prevent UI clutter
    // The mode is already shown in the selection controls panel
    console.log('✨ Mode indicator creation skipped for cleaner UI');
  }
  ...
  handleManualModeToggle(isManual) {
    ...
    // Mode status is displayed in the selection controls panel
