/*
 * FlowMounds (v0.25), on the wall -- the still creature.
 *
 ...
 *
 * What the wall adds is time. The painting builds itself in front of the room,
 * and when it is finished the face wakes up: it blinks, its eyes drift, and it
 * rides the bass. The paint underneath is never touched, so what hangs there is
 * still the explorer's picture. (One difference, for honesty: the face is drawn
 * last here, so a rare effect that belongs over it would sit under it instead.
 * The look this piece ships has no rare effect.)
 *
 * The room steers the painting. The host picks which mound.
 */
...
// ── the still creature, with a face that is awake ───────────────────────────
// The painting is the explorer's, finished and left alone. The face is the one
// thing drawn every frame: it blinks, its eyes drift, and it rides the bass.
// Nothing it does touches the paint underneath.
const WANDER = 180;          // seconds on one mound before wandering off it
let faceKey = '';
let blinkAt = -9, nextBlink = 1e9, bass = 0, lastPreview = -1e9;

// A face only comes alive at the size a wall is. In a thumbnail on the desk
// an eye is ten pixels across, nobody can see it blink, and drawing it every
// frame is most of the frame -- so there it is painted in and left still.
const lively = () => tier !== 'low';

function onFinished(t) {
  doneAt = t;
  composite({ face: !lively() });      // when it is alive, the face goes on top
  nextBlink = t + 1.9;                 // it wakes up, and blinks
}

function animation(t, dt, audio) {
  const alive = clamp((t - doneAt - 0.3) / 1.5, 0, 1);
  if (alive <= 0 || !facePlan) return null;
  bass += (((audio && audio.bass) || 0) - bass) * Math.min(1, dt * 5);
  if (t > nextBlink) { blinkAt = t; nextBlink = t + 3 + Math.random() * 4.5; }
  else if (audio && audio.beat && t - blinkAt > 2.2 && Math.random() < 0.25) {
    blinkAt = t;
    nextBlink = t + 3 + Math.random() * 4.5;
  }
  const b = t - blinkAt;
  const open = b < 0.08 ? 1 - (b / 0.08) * 0.92
    : b < 0.13 ? 0.08
    : b < 0.28 ? 0.08 + ((b - 0.13) / 0.15) * 0.92 : 1;
  return {
    open: 1 - (1 - open) * alive,
    dx: alive * facePlan.ew * 0.10 * Math.sin(t * 0.43) * Math.cos(t * 0.19 + 1.2),
    dy: alive * (facePlan.eh * 0.06 * Math.sin(t * 0.31 + 0.7) - facePlan.eh * 0.10 * bass),
  };
}

function startPicture(want, m, t) {
  moundNo = m;
  doneAt = 1e9;                        // nothing is alive until it is finished
  allocBuffers();
  build(explorerParams(want, m), 0);
  builtKey = keyOf(want, m);
  faceKey = `${want.eye_size}|${want.eye_shape}`;
  built = true;
  lastPreview = -1e9;
}

function frame_(ctx, frame, getVar, audio) {
  const t = frame.t;
  ...
  const m = mounded(want, t, want.wander, WANDER);
  if (!built) startPicture(want, m, t);
  else repaintWhenSettled(ctx, want, m, t, startPicture);

  // The face's own two controls move it without repainting: it is planned
  // before a single mark is laid, and nothing in the painting depends on it.
  const fk = `${want.eye_size}|${want.eye_shape}`;
  if (fk !== faceKey) {
    faceKey = fk;
    if (!replanFace(want.eye_size, want.eye_shape)) builtKey = '~repaint';
  }

  if (!finished) {
    advance(BUDGET_MS);
    if (finished) onFinished(t);
    else if (t * 1000 - lastPreview > PREVIEW_MS) { lastPreview = t * 1000; composite({ face: !lively() }); }
  }

  showSquare(ctx, lively() ? sceneWithFace(animation(t, frame.dt || 1 / 60, audio)) : art.canvas);
  showFade(ctx, t);
}
